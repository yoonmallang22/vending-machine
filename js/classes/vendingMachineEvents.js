class VendingMachineEvents {
  ...
  bindEvent() {
        ...
        if (balanceVal >= targetElPrice) {
          ...
          for (const item of stagedListItem) {
            ...
            if (targetEl.dataset.item === item.dataset.item) {
              ...
              item.querySelector("strong").firstChild.textContent = parseInt(item.querySelector("strong").textContent) + 1;
     ...
     * 2) 획득한 음료의 모든 금액을 합하여 총금액을 업데이트
     */
      ...
      for (const itemStaged of itemStagedList) {
        ...
        for (const itemGet of itemGetList) {
          ...
          if (itemStaged.dataset.item === itemGet.dataset.item) {
            // 이미 장바구니에 콜라가 있다면 카운트 +
            itemGet.querySelector("strong").firstChild.textContent = parseInt(itemGet.querySelector("strong").textContent) + parseInt(itemStaged.querySelector("strong").textContent);
